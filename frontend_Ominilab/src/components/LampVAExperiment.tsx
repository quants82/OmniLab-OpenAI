...
function LampVAContent() {
    ...
    const toggleConnection = useCallback(() => {
        ...
        socket.onmessage = (event) => {
            let data = event.data;
            if (data.startsWith("RESULT=")) data = data.substring(7);
            try {
    ...
    const calculateCurveFit = (points: RecordPoint[]) => {
        const validPoints = points.filter(p => p.u > 0.1 && p.i > 0);
        if (validPoints.length < 2) return null;
        const dataForReg = validPoints.map(p => [p.u, p.i]);
        const result = regression.power(dataForReg, { precision: 10 });
        const lastP = points[points.length - 1];
        const R_instant = lastP.i > 0.1 ? (lastP.u / (lastP.i / 1000)) : 0;
        const maxRecordedU = Math.max(...points.map(p => p.u), 4);
        const linePoints = [{ x: 0, y: 0 }];
        for (let u = 0.1; u <= maxRecordedU + 0.1; u += 0.1) {
            const predY = result.predict(u)[1];
            linePoints.push({ x: u, y: isNaN(predY) ? 0 : predY });
        }
        return { R_instant, linePoints, r2: result.r2 };
    };

    const currentAnalysis = useMemo(() => calculateCurveFit(recordedPoints), [recordedPoints]);

    const saveCurrentCurve = () => {
        if (!currentAnalysis) return;
        setSavedExp({ points: [...recordedPoints], rValue: currentAnalysis.R_instant, linePoints: currentAnalysis.linePoints });
        setRecordedPoints([]);
    };

    const scatterData = {
        datasets: [
            savedExp && { label: `Previous characteristic (R ≈ ${savedExp.rValue.toFixed(1)} Ω)`, data: savedExp.linePoints, borderColor: '#94a3b8', borderWidth: 2, pointRadius: 0, showLine: true, zIndex: 4 },
            { label: `Current measurements`, data: recordedPoints.map(p => ({ x: p.u, y: p.i })), backgroundColor: '#f43f5e', pointRadius: 8, zIndex: 10 },
            currentAnalysis && { label: 'Lamp characteristic (nonlinear)', data: currentAnalysis.linePoints, borderColor: '#10b981', borderWidth: 4, pointRadius: 0, showLine: true, zIndex: 8 },
        ...
        <div className="min-h-screen bg-[#fdfcf0] text-slate-800 p-4 md:p-6 pb-20 flex flex-col gap-6 font-sans">
            ...
            <header className="max-w-7xl mx-auto w-full flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                <div className="flex flex-col">
                    <h1 className="text-3xl font-bold text-slate-900 uppercase tracking-tight">Incandescent Lamp I-V Characteristic</h1>
            ...
            <main className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-6">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            <div className="p-5 bg-amber-50 rounded-3xl border border-amber-100 flex flex-col items-center">
                                ...
                                <div className="w-full h-1.5 bg-amber-200/30 rounded-full mt-4 overflow-hidden">
                                    <div className="h-full bg-amber-500 rounded-full transition-all" style={{ width: `${Math.min(100, (liveVal.u / 6.0) * 100)}%` }}/>
                                </div>
                            </div>
                        ...
                        <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white border border-slate-800 shadow-xl space-y-6 relative overflow-hidden group">
                            ...
                            <div className="relative z-10 space-y-6">
                                <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-500 flex items-center gap-2 border-b border-white/5 pb-2">
                                    <Activity size={14} className="text-indigo-400"/> Survey result
                                </h3>
                                <div className="flex flex-col gap-2">
                                    <span className="text-[10px] font-bold text-indigo-400 uppercase">Instantaneous R (filament)</span>
                                    <div className="text-5xl font-black text-indigo-400 font-mono tracking-tighter">{currentAnalysis.R_instant.toFixed(1)}<small className="text-xl ml-1 uppercase">Ω</small></div>
                                    <p className="text-[9px] text-slate-500 italic mt-2">Metal resistance increases with temperature (nonlinear).</p>
                                </div>
                                <button onClick={saveCurrentCurve} className="w-full py-3 bg-white/5 border border-white/10 rounded-2xl text-[9px] font-black uppercase hover:bg-white/10 transition-all">
                ...
                <div className="lg:col-span-8 flex flex-col gap-6">
                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm h-[500px] flex flex-col gap-4 relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-12 opacity-[0.02] rotate-12 pointer-events-none"><Lightbulb size={300}/></div>
                        <h3 className="text-sm font-black text-slate-800 uppercase tracking-wide flex items-center gap-2">
                            <Activity size={18} className="text-rose-500" /> Incandescent lamp I–V characteristic
                        </h3>
                        <div className="flex-1 w-full relative">
                            <Scatter
                                ...
                                options={{
                                    ...
                                    scales: {
                                        x: { type: 'linear', min: 0, title: { display: true, text: 'Voltage U (V)', font: { weight: 'bold' } }, grid: { color: '#f8fafc' } },
                    ...
                    <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col flex-1 h-[400px]">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <h3 className="font-bold text-slate-700 uppercase text-[10px] tracking-widest flex items-center gap-2"><Disc size={16} className="text-blue-500"/> Detailed data table</h3>
                            ...
                            <table className="w-full text-left border-collapse text-xs">
                                <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-100">
                                    <tr className="text-[10px] font-black uppercase text-slate-400">
                                        <th className="p-5 italic pl-8">Reading</th>
...
export default function LampVAExperiment() {
    return <AuthProvider><ProtectedLab><LampVAContent /></ProtectedLab></AuthProvider>;
}
